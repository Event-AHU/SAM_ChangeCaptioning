...
def main(args):
    ...
    if args.data_name == 'LEVIR_CC':
        ...
        test_loader = data.DataLoader(
            DubaiCCDataset(args.data_folder, args.list_path, 'test', args.token_folder, args.vocab_file, args.max_length, args.allow_unk),
            batch_size=args.test_batchsize, shuffle=False, num_workers=args.workers, pin_memory=True)
    ...
    def tokens_to_words(tokens):
                return ' '.join([list(word_vocab.keys())[i] for i in tokens if i not in {word_vocab['<START>'], word_vocab['<END>'], word_vocab['<NULL>']}])


    predictions_file = open(os.path.join(args.savepath, 'predictions.txt'), 'w')
    ...
    with torch.no_grad():
        ...
        hypotheses = []   # 预测描述
        # Batches
        for ind, (imgA, imgB, token_all, token_all_len, _, _, name,fineA, fineB,semanticA,semanticB) in enumerate(test_loader):
            ...
            seq = decoder.sample(feat1, feat2, fineA, fineB, semanticA, semanticB,imgA, imgB, k=1)
   ...
   # 使用之前定义的tokens_to_words函数转换token 
            actual_caption = tokens_to_words(img_tokens[0])
            predicted_caption = tokens_to_words(pred_seq)

        # 写入文件
            predictions_file.write(f"Image {ind+1}: {name}\n") 
            predictions_file.write(f"Actual: {actual_caption}\n")
            predictions_file.write(f"Predicted: {predicted_caption}\n\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--node_features', default="levir50_extracted_node_features.pt",help='base name shared by data files.')
    parser.add_argument('--edge_index', default="levir50_edge_index.pt",help='base name shared by data files.')
    parser.add_argument('--edge_type', default="levir50_edge_type.pt",help='base name shared by data files.')
    
    # Data parameters
    parser.add_argument('--data_folder', default='/LEVIR_CC/images',help='folder with data files')
    parser.add_argument('--list_path', default='/LEVIR_CC/', help='path of the data lists')
    parser.add_argument('--token_folder', default='/LEVIR_CC/tokens/', help='folder with token files')
    ...
    parser.add_argument('--checkpoint', default='checkpoint', help='path to checkpoint, None if none.')
    ...
    parser.add_argument('--savepath', default="/prediction/")
