...
def main(args):
    ...
    encoder_trans = encoder_trans.to('cuda:0')
    ...
    for epoch in range(start_epoch, args.num_epochs):        
        for id, (imgA, imgB, _, _, token, token_len, name, fineA, fineB, semanticA, semanticB) in enumerate(train_loader):
            ...
            scores, caps_sorted, decode_lengths, sort_ind = decoder(feat1, feat2, token, token_len, fineA, fineB, semanticA, semanticB)
        ...
        with torch.no_grad():
            for ind, (imgA, imgB, token_all, token_all_len, _, _, _, fineA, fineB, semanticA, semanticB) in enumerate(val_loader):
                imgA = imgA.cuda()
                imgB = imgB.cuda()
                if args.data_name == 'Dubai_CC':
                    imgA = l_resizeA(imgA)
            ...
            print('Validation:\nTime: {0:.3f}\tBLEU-4: {1:.4f}'.format(val_time, Bleu_4))
        ...
        if  Bleu_4 > best_bleu4:
            ...
            # print("Shape1 of the tensor:", feat1.shape)
            # print("Shape2 of the tensor:", feat2.shape)
            print('Final encoder outputs saved.')
            #save_checkpoint                
            print('Save Model')  
...
if __name__ == '__main__':
    ...
    parser.add_argument('--node_features', default="extracted_node_features.pt",help='base name shared by data files.')
    parser.add_argument('--edge_index', default="edge_index.pt",help='base name shared by data files.')
    parser.add_argument('--edge_type', default="edge_type.pt",help='base name shared by data files.')
    ...
    parser.add_argument('--savepath', default="your savepath")
    # Model parameters
